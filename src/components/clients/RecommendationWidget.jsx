...
const RecommendationWidget = ({ type = "articles" }) => {
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 mb-8">
        {[1, 2, 3].map((n) => (
          <div
            key={n}
            className="animate-pulse bg-neutral-100 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-800 rounded-lg h-72"
          />
        ))}
      </div>
    );
  ...
  return (
    <motion.section
      ...
      className="mb-8 p-5 rounded-lg bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 shadow-sm"
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5 pb-4 border-b border-neutral-100 dark:border-neutral-800">
        <div className="flex items-center gap-2.5">
          <div className="p-1.5 bg-brand-500/10 dark:bg-brand-500/5 text-brand-500 rounded">
            {meta.icon}
          </div>
          <div>
            <h2 className="text-sm font-semibold text-neutral-900 dark:text-white tracking-tight flex items-center gap-2">
              {meta.title}
        ...
        {type === "tests" && message && (
          <div className="flex items-center gap-1.5 px-2.5 py-1 bg-yellow-500/10 dark:bg-yellow-500/5 border border-yellow-500/10 text-yellow-600 dark:text-yellow-400 rounded text-[11px] font-medium self-start sm:self-center">
            <AlertCircle size={12} />
            <span>{message}</span>
          </div>
        )}
