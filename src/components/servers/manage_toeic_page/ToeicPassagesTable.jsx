...
const ToeicPassagesTable = ({ passages, onEdit, onDelete }) => {
  ...
  const getAudioUrl = (passage) => passage.audioUrl || passage.audio_url || passage.audio_request?.uploaded_audio || passage.audio_request?.audio_url;
  const getImageUrl = (passage) => passage.imageUrl || passage.image_url || passage.image_request?.uploaded_image || passage.image_request?.image_url;
        ...
        <table className="w-full whitespace-nowrap">
          <thead>
            <tr className="bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">
              ...
              <th className="px-6 py-4 min-w-[200px]">Passage Text (Preview)</th>
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
            {passages.map((passage) => (
              <tr key={passage.id} className="hover:bg-gray-50 dark:hover:bg-gray-750 transition-colors">
                <td className="px-6 py-4 text-sm font-medium text-gray-900 dark:text-gray-200">{passage.id}</td>
                ...
                <td className="px-6 py-4 text-sm">
                  <div className="flex gap-2 items-center">
                    {getImageUrl(passage) && <ImageIcon size={18} className="text-blue-500" title="Has Image" />}
                    {getAudioUrl(passage) && (
                      <button
                        ...
                        title="Play Audio"
                      >
                        <Volume2 size={18} className={playingId === passage.id ? "animate-pulse" : ""} />
                      </button>
                    )}
                    {!getImageUrl(passage) && !getAudioUrl(passage) && <span className="text-gray-400 italic text-xs">None</span>}
                  </div>
                </td>
