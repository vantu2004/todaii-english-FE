...
const AdminViewModal = ({ isOpen, onClose, admin }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Admin Profile" width="sm:max-w-2xl">
      <div className="space-y-6">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-1">{admin.display_name}</h3>
        ...
        <div className="border-t border-gray-100 dark:border-gray-800 pt-5">
          <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Roles & Permissions</h4>
          <div className="flex flex-wrap gap-2">
            {admin.roles?.length > 0 ? (
              admin.roles.map((role) => (
                <span key={role.code} className="px-2.5 py-1 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 rounded-md text-sm font-medium">
                  {role.description}
                </span>
              ))
            ) : (
        ...
        <div className="border-t border-gray-100 dark:border-gray-800 pt-5">
          ...
          <div className="space-y-3">
            <InfoRow label="Account Created" value={formatISODate(admin.created_at)} />
            <InfoRow label="Last Updated" value={formatISODate(admin.updated_at)} />
            <InfoRow label="Last Login" value={formatISODate(admin.last_login_at)} />
          </div>
        </div>
        ...
        <div className="border-t border-gray-100 dark:border-gray-800 pt-5">
          <div className="grid grid-cols-2 gap-4">
            <div className="border border-gray-200 dark:border-gray-800 p-4 rounded-lg">
              ...
              <div className="text-lg font-semibold text-gray-900 dark:text-white">{admin.status}</div>
            </div>
            <div className="border border-gray-200 dark:border-gray-800 p-4 rounded-lg">
              <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">Total Roles</div>
              <div className="text-lg font-semibold text-gray-900 dark:text-white">{admin.roles?.length || 0}</div>
            </div>
          </div>
        </div>
      </div>
