...
const UserViewModal = ({ isOpen, onClose, user }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="User Profile" width="sm:max-w-2xl">
      <div className="space-y-6">
        ...
        <div className="flex items-start gap-4">
          <div className="relative">
            ...
            <div className={`absolute -bottom-1 -right-1 ${status.bg} ${status.color} px-2 py-0.5 rounded-md text-xs font-medium border-2 border-white dark:border-gray-900`}>
              {status.label}
            </div>
          </div>
          <div className="flex-1">
            <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-1">{user.display_name}</h3>
        ...
        <div className="border-t border-gray-100 dark:border-gray-800 pt-5">
          <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">User Role</h4>
          <div className="flex items-center gap-3">
            <span className="px-2.5 py-1 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 rounded-md text-sm font-medium">Standard User</span>
            <span className="text-xs text-gray-500 dark:text-gray-400 italic">Default role for all registered users</span>
          </div>
        </div>
        ...
        <div className="border-t border-gray-100 dark:border-gray-800 pt-5">
          ...
          <div className="space-y-3">
            <InfoRow label="Account Created" value={formatISODate(user.created_at)} />
            <InfoRow label="Last Updated" value={formatISODate(user.updated_at)} />
            <InfoRow label="Last Login" value={formatISODate(user.last_login_at)} />
          </div>
        </div>
        ...
        <div className="border-t border-gray-100 dark:border-gray-800 pt-5">
          <div className="grid grid-cols-2 gap-4">
            <div className="border border-gray-200 dark:border-gray-800 p-4 rounded-lg">
              ...
              <div className="text-lg font-semibold text-gray-900 dark:text-white">{user.status}</div>
            </div>
            <div className="border border-gray-200 dark:border-gray-800 p-4 rounded-lg">
              <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">User Type</div>
              <div className="text-lg font-semibold text-gray-900 dark:text-white">Standard</div>
            </div>
          </div>
        </div>
      </div>
