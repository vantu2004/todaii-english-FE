import { Volume2, BookOpen, Layers, Target } from "lucide-react";
        ...
        <div className="border border-gray-200 rounded-lg p-5 bg-gray-50">
          <div className="flex items-start justify-between">
            ...
            <div className="flex gap-2">
              {result?.pronounce?.us && (
                <button
                  onClick={() => handlePlayAudio(result.pronounce.us)}
                  className="p-2 bg-gray-900 text-white rounded-lg hover:bg-gray-800 flex items-center gap-2"
                >
              ...
              {result?.pronounce?.gb && (
                <button
                  onClick={() => handlePlayAudio(result.pronounce.gb)}
          ...
          {result?.level_word && (
            <div className="mt-4 flex gap-3 text-sm">
              {result.level_word.toeic && (
                <span className="px-2.5 py-1 bg-blue-100 text-blue-700 rounded-md font-medium border border-blue-200 flex items-center gap-1">
                  <Target className="w-4 h-4" /> TOEIC:{" "}
                  {result.level_word.toeic}
                </span>
              )}
              {result.level_word.vietnam && (
                <span className="px-2.5 py-1 bg-green-100 text-green-700 rounded-md font-medium border border-green-200">
        ...
        {result?.word_family?.length > 0 && (
          <div>
            <h4 className="font-bold text-gray-800 flex items-center gap-2 mb-3">
              <Layers className="w-5 h-5 text-gray-500" /> Word Family
            </h4>
