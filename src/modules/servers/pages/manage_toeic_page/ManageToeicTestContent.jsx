...
const ManageToeicTestContent = () => {
  ...
  // Sub-screen for passage questions state
  const [selectedPassage, setSelectedPassage] = useState(null);

  // Reset selected passage when part changes
  useEffect(() => {
  ...
  const handleConfirmDeletePassage = async () => {
    ...
    try {
      ...
      if (selectedPassage?.id === passageToDelete) {
        setSelectedPassage(null);
      }
      loadContentForPart(activePart);
    } catch (err) {
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-gray-800 rounded-lg p-6 overflow-y-auto">
      {/* Tabs - Hidden when viewing passage questions */}
      {!selectedPassage && (
      ...
      {loading ? (
        ...
        <motion.div
          key={activePart + (selectedPassage ? "-passage" : "-main")}
        ...
        >
          {PARTS_WITH_PASSAGE.includes(activePart) ? (
            selectedPassage ? (
                ...
                <div className="flex items-center justify-between mb-6 bg-slate-50 dark:bg-gray-750 p-4 rounded-xl border border-slate-200 dark:border-gray-700 shadow-sm">
                  <div>
                    <button
                      ...
                      className="text-sm font-semibold text-blue-600 dark:text-blue-400 hover:text-blue-800 transition flex items-center gap-1 mb-2"
                    >
                      &larr; Back to Passages list
                    </button>
                    <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100">
                      Questions for Passage #{selectedPassage.id}
                    </h3>
                    <p
                      className="text-xs text-gray-500 dark:text-gray-400 mt-1 max-w-xl truncate italic"
                      dangerouslySetInnerHTML={{
                        __html: selectedPassage.passageText || "No text",
                      }}
                    />
                  </div>
                  <button
              ...
              /* Main screen: Passages list only */
              <div>
                ...
                <ToeicPassagesTable
                  ...
                  onManageQuestions={(passage) => setSelectedPassage(passage)}
                />
              </div>
            )
          ) : (
            /* Parts without passage (Part 1, 2, 5) */
            <div>
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-xl font-bold text-gray-800 dark:text-gray-100">
      ...
      {isQuestionModalOpen && (
        <ToeicQuestionFormModal
          ...
          passages={passages}
          tags={tags}
          defaultPassageId={selectedPassage?.id}
          onSuccess={() => {
