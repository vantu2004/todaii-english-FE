...
const ResendOtp = () => {
  ...
  const handleResend = async () => {
    try {
      // Gọi API gửi lại OTP (placeholder)
      // await resendOtp(email);
      toast.success(`OTP has been resent to ${email}`);
      navigate("/server/verify-otp");
    } catch (err) {
      ...
      >
        ...
        <div className="flex flex-col items-center text-center space-y-3">
          <div className="flex items-center justify-center space-x-2 text-indigo-400">
            <Mail size={26} />
            <h1 className="text-2xl font-semibold tracking-wide">Resend OTP</h1>
          </div>
