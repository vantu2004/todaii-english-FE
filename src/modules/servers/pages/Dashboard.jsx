...
import RedisDashboard from "@/components/servers/dashboard/RedisDashboard";
...
const Dashboard = () => {
  ...
  // Top-level View switcher: overview | redis
  ...
  useEffect(() => {
    setHeader({
      ...
      breadcrumb: [
        ...
        { label: "Dashboard", to: "/server" },
        ...(dashboardView === "redis" ? [{ label: "Redis" }] : []),
      ],
    });
  }, [dashboardView]);
    ...
    <div className="flex flex-col space-y-6 min-h-full pb-10">
      ...
      <div className="flex border-b border-gray-200 dark:border-gray-800 pb-3">
        <div className="flex bg-gray-100 dark:bg-gray-850 p-1 rounded-lg">
          ...
          <button
            onClick={() => setDashboardView("redis")}
            className={`px-4 py-2 text-xs font-medium rounded-md transition-colors ${
              dashboardView === "redis"
                ? "bg-white dark:bg-gray-900 text-gray-900 dark:text-white shadow-sm"
                : "text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white"
